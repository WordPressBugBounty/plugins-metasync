...
<?php
    ...
    public function get_error_logs($limit = -1)
    {
        ...
        // Initialize an array to store the structured logs.
        $structuredLogs = [];

        foreach ($contents as $key => $value) {
            // Use regular expressions to extract relevant information from each log entry.
            preg_match("/\[(.*?)\] (.*) (.*?): (.*)/", $value, $matches);

            if (count($matches) == 5) {
                // Create an associative array with extracted information.
                $logData = [
                    'id' => $key + 1,
                    'timestamp' => $matches[1],
                    'severity' => $matches[3],
                ...
                // Add the log entry to the structuredLogs array.
                $structuredLogs[] = $logData;
            }
        }

        // If no limit is specified, return all logs, otherwise, return the last $limit entries.
        $logsToReturn = ($limit === -1) ? $structuredLogs : array_slice($structuredLogs, -$limit);
        // Call the get_error_logs method to obtain the JSON representation of error logs.
        // Convert the structured logs array to JSON format.
        return wp_json_encode($logsToReturn);
    }
    ...
    public function style_my_table()
    {
        ...
        echo '.manage-column.column-id { width: 10%; }';
        echo '.manage-column.column-timestamp { width: 15%; }';
        echo '.manage-column.column-severity  { width: 15%; }';
        echo '.manage-column.column-message { width: 60%; }';
        echo '</style>';
    }
}
